...
class Debugging:
    ...
    def __init__(self, *, dir_name=None) -> None:
        ...
        self.except_dirs, self.except_exts, self.verified_files, self.proj_size, self.str_len =\
    ...
    def __check_dir_size__(self, dir_name) -> None:
        ...
        for root, dirs, files in os.walk(dir_name):
            ...
            for file in files:
                ...
                if file not in self.verified_files:
                    ...
                    if file_ext[1:] not in self.except_exts:

                        try:
                            with open(path, mode='r', encoding='utf8') as f:
                                f_len = len(f.readlines())
                                # print(f"   length = {f_len}", end=' ,')
                                self.proj_size['lines'] += f_len
            ...
            for directory in dirs:
                ...
                self.__check_dir_size__(abs_directory)

                self.except_dirs.add(directory)
    ...
    def get_proj_size(self, except_dirs=None, except_exts=None, except_files=None) -> dict:
        ...
        [self.verified_files.add(except_file) for except_file in except_files]\
            if except_files is not None else None

        # directories that are excluded during the check
        self.except_dirs: set = {'.idea', '__pycache__', 'venv', '.git'}
        [self.except_dirs.add(except_dir) for except_dir in except_dirs]\
            if except_dirs is not None else None

        # files with extensions that are skipped during verification
        self.except_exts: set = {'jpg', 'png', 'mp3', 'ogg', 'obj', 'docx',  # various formats
                                 'rar', 'zip',
                                 'pyc', 'pyz', 'pkg',
                                 'ico', 'lnk', 'bin', 'exe', 'dll'}
        [self.except_exts.add(except_ext) for except_ext in except_exts]\
            if except_exts is not None else None

        # first call recursive function
        self.__check_dir_size__(self.directory)
